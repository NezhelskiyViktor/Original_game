...
class MiniMenu:
    ...
    def run(self, diffi_level, live_count, game_state, sound, music, sound_volume, music_volume):
        self.difficulty_index = diffi_level
        self.lives_index = live_count
        self.game_state_index = game_state
        self.sound_index = sound
        self.music_index = music
        self.s_v_index = sound_volume
        self.m_v_index = music_volume
        ...
        return (self.difficulty_index, self.lives_index, self.game_state_index,
                self.sound_index, self.music_index, self.s_v_index, self.m_v_index)
    ...
    def draw(self):
        ...
        self.draw_text('Звуки', self.font18, red, self.subsurface, 10, 95)
        self.draw_text('Музыка', self.font18, red, self.subsurface, 10, 125)
        self.draw_text('Громкость звуков', self.font18, red, self.subsurface, 10, 155)
        self.draw_text('Громкость музыки', self.font18, red, self.subsurface, 10, 185)
        ...
        self.draw_text(self.sound[self.sound_index], self.font18,
                       yellow if self.active_index == 3 else red, self.subsurface, 120, 95)
        self.draw_text(self.music[self.music_index], self.font18,
                       yellow if self.active_index == 4 else red, self.subsurface, 120, 125)
        self.draw_text(str(self.sound_volume[self.s_v_index]), self.font18,
                       yellow if self.active_index == 5 else red, self.subsurface, 200, 155)
        self.draw_text(str(self.music_volume[self.m_v_index]), self.font18,
                       yellow if self.active_index == 6 else red, self.subsurface, 200, 185)
